  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 font-mono">
        <motion.div 
            ...
            className="w-full max-w-md bg-black border border-yellow-500/50 shadow-[0_0_40px_rgba(234,179,8,0.1)] relative rounded-sm overflow-hidden"
        >
            {/* Header */}
            <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-yellow-500/5">
                <h2 className="text-lg font-black italic text-yellow-500 uppercase flex items-center gap-2 tracking-tighter">
                    <HandCoins className="w-5 h-5" /> GOLD STORE
                </h2>
            ...
            <div className="p-6">
                ...
                <div className="mb-6 space-y-3">
                    <label className="text-[10px] text-gray-500 font-bold block uppercase tracking-widest">Select Package</label>
                    {GOLD_PACKAGES.map((pkg) => (
                        <button
                            key={pkg.id}
                            onClick={() => setSelectedPack(pkg)}
                            className={`w-full flex justify-between items-center p-4 border transition-all duration-200 group ${
                        ...
                        >
                            <div className="text-left flex items-center gap-3">
                                <div className={`w-8 h-8 flex items-center justify-center rounded-sm ${selectedPack.id === pkg.id ? 'bg-yellow-500 text-black' : 'bg-gray-900 text-gray-600'}`}>
                                    <Zap className="w-4 h-4 fill-current" />
                                </div>
                                <div>
                                    <div className={`font-black uppercase text-sm ${selectedPack.id === pkg.id ? 'text-white' : 'text-gray-400'}`}>{pkg.gold} GOLD</div>
                                    <div className="text-[9px] text-gray-500 uppercase tracking-wider">{pkg.label}</div>
                                </div>
                            </div>
                            <div className="text-right">
                                ...
                                {pkg.popular && <span className="text-[9px] bg-yellow-500 text-black px-1.5 py-0.5 rounded-sm font-bold uppercase">HOT</span>}
                            </div>
                        </button>
                    ))}
                </div>
                    ...
                    {['paypal', 'crypto', 'upi'].map((m) => (
                        <button 
                            ...
                            className={`p-3 border flex flex-col items-center gap-2 transition-colors ${
                                ...
                                : 'border-gray-800 text-gray-600 hover:border-white hover:text-white'
                            }`}
                        >
                ...
                {paymentMethod === 'paypal' && (
                    <a href={selectedPack.paypalLink} target="_blank" rel="noopener noreferrer" className="w-full py-4 bg-yellow-500 hover:bg-white text-black font-black uppercase rounded-sm flex items-center justify-center gap-2 transition-colors tracking-widest">
                        <CreditCard className="w-5 h-5" /> PAY ${selectedPack.cost.usd}
                    </a>
                )}
                
                {paymentMethod === 'crypto' && (
                    <button onClick={handleCryptoPayment} disabled={isLoading || !isConnected} className="w-full py-4 bg-[#39FF14] hover:bg-white text-black font-black uppercase rounded-sm flex items-center justify-center gap-2 transition-colors disabled:opacity-50 tracking-widest">
                        {isLoading ? <Loader2 className="animate-spin" /> : <><Droplet className="w-5 h-5" /> SEND {selectedPack.cost.eth} ETH</>}
                    </button>
                )}

                {paymentMethod === 'upi' && (
                    <button onClick={handleUpiPayment} className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black uppercase rounded-sm flex items-center justify-center gap-2 transition-colors tracking-widest">
