...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('API: Create PayPal order API route caught error:', error.message || error);
    return new Response(JSON.stringify({ error: error.message || 'Error creating PayPal order' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}