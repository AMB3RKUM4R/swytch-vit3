...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('API: Capture PayPal order API route caught error:', error.message || error);
    return new Response(JSON.stringify({ success: false, error: error.message || 'Error capturing PayPal order' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}