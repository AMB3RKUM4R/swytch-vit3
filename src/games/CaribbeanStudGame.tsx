...
const getHandRank = (cards: Card[]): { rank: number; name: string } => {
  ...
  const suitCounts = suits.reduce((acc: { [key: string]: number }, suit) => {
  ...
  const isStraight = sortedValues.every((val, i) => i === 0 || valueOrder.indexOf(val) === valueOrder.indexOf(sortedValues[i - 1]) + 1);

  if (isFlush && sortedValues[0] === '10' && sortedValues.includes('A')) return { rank: 10, name: 'Royal Flush' };
  if (isFlush && isStraight) return { rank: 9, name: 'Straight Flush' };
  if (Object.values(valueCounts).some(count => count === 4)) return { rank: 8, name: 'Four of a Kind' };
  if (Object.values(valueCounts).some(count => count === 3) && Object.values(valueCounts).some(count => count === 2)) return { rank: 7, name: 'Full House' };
  if (isFlush) return { rank: 6, name: 'Flush' };
  if (isStraight) return { rank: 5, name: 'Straight' };
  if (Object.values(valueCounts).some(count => count === 3)) return { rank: 4, name: 'Three of a Kind' };
  if (Object.values(valueCounts).filter(count => count === 2).length === 2) return { rank: 3, name: 'Two Pair' };
  if (Object.values(valueCounts).some(count => count === 2)) return { rank: 2, name: 'One Pair' };
  return { rank: 1, name: `High Card ${sortedValues[sortedValues.length - 1]}` };
};
...
const CaribbeanStudGame: React.FC<CaribbeanStudGameProps> = ({ userId, setIsPETMember, updatePlayerFirestore }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        let roomId = roomsSnap.docs.find((doc: QueryDocumentSnapshot) => doc.data().status === "waiting" && doc.data().game === "caribbean-stud")?.id;
        if (!roomId) {
  ...
  useEffect(() => {
    if (effectiveUserId) {
      updatePlayerFirestore({
        ...
        quests,
        achievements,
        caribbeanStudPlays: stats.plays,
        caribbeanStudWins: stats.wins,
        caribbeanStudLosses: stats.losses,
        caribbeanStudBiggestWin: stats.biggestWin,
      }).catch((err) => {
  ...
  const shareWinOnX = async (): Promise<void> => {
    ...
    const shareQuest = quests.find((q) => q.id === "caribbeanStud-share");
  ...
  const playGame = async (): Promise<void> => {
    ...
    const deck = shuffleDeck(Date.now().toString());
    ...
    setTimeout(async () => {
      const playerRank = getHandRank(player);
      const dealerRank = getHandRank(dealer);
      const won = playerRank.rank > dealerRank.rank;
      const payout = won ? betAmount * 2 : 0;
      const result = won ? `You Win! (${playerRank.name} beats ${dealerRank.name})` : `Dealer Wins! (${dealerRank.name} beats ${playerRank.name})`;
      ...
      if (payout > 0) {
        ...
        setShowReward({ jewels: payout, xp: 10, message: `Caribbean Stud! You won +${payout} ${useJewels ? "JEWELS" : "USDT"}! ${result}` });
        if (winSoundRef.current) winSoundRef.current.play().catch((err) => console.error("Audio playback failed:", err));

        const winQuest = quests.find((q) => q.id === "caribbeanStud-wins");
        if (winQuest && !winQuest.completed) {
          ...
          setQuests((prev) =>
            prev.map((q) => (q.id === "caribbeanStud-wins" ? { ...q, progress: newProgress, completed: isQuestCompleted } : q))
          );
          if (isQuestCompleted) {
        ...
        const achievement = achievements.find((a) => a.id === "caribbeanStud-master");
        if (achievement && !achievement.unlocked && newStats.wins >= 10) {
          setAchievements((prev) => prev.map((a) => (a.id === "caribbeanStud-master" ? { ...a, unlocked: true } : a)));
          setJewels((prev) => prev + 20);
          setShowReward({ jewels: 20, xp: 30, message: "Achievement Unlocked: Caribbean Stud Master!" });
      ...
      const playQuest = quests.find((q) => q.id === "caribbeanStud-play");
      if (playQuest && !playQuest.completed) {
        ...
        setQuests((prev) =>
          prev.map((q) => (q.id === "caribbeanStud-play" ? { ...q, progress: newProgress, completed: isQuestCompleted } : q))
        );
        if (isQuestCompleted) {
      ...
      setGameState('RESULT');
      await updatePlayerFirestore({ caribbeanStudPlays: newStats.plays, caribbeanStudWins: newStats.wins, caribbeanStudLosses: newStats.losses, caribbeanStudBiggestWin: newStats.biggestWin });
  ...
  return (
    <section className="relative py-16 px-6 sm:px-8 lg:px-16 bg-gradient-to-br from-gray-950 via-rose-950/20 to-black text-white overflow-hidden font-inter">
      {gameRoom.phase === 'RESULT' && gameRoom.result.includes('You Win') && (
      ...
      >
                  ...
                  <div className="flex gap-2 mb-4">
                    <SwytchErrorBoundary>
                      <Canvas style={{ height: "100px" }} camera={{ position: [0, 0, 5], fov: 50 }}>
                        ...
                        {gameRoom.playerHands[effectiveUserId!].hand.map((card, index) => (
                          <Card3D key={`${card.suit}-${card.value}-${index}`} card={card} position={[index * 1 - (gameRoom.playerHands[effectiveUserId!].hand.length - 1) / 2, 0, 0]} />
                        ))}
                      </Canvas>
                    </SwytchErrorBoundary>
                  </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    <SwytchErrorBoundary>
                      <Canvas style={{ height: "100px" }} camera={{ position: [0, 0, 5], fov: 50 }}>
                        <ambientLight intensity={0.5} />
                        <pointLight position={[10, 10, 10]} intensity={1} />
                        {gameRoom.dealerHand.map((card, index) => (
                          <Card3D key={`${card.suit}-${card.value}-${index}`} card={card} position={[index * 1 - (gameRoom.dealerHand.length - 1) / 2, 0, 0]} />
                        ))}
                      </Canvas>
                    </SwytchErrorBoundary>
                  </div>
        ...
        <motion.div variants={sectionVariants} className="text-center mb-12">
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            aria-label="Show Caribbean Stud Tutorial"
          >
        ...
        <AnimatePresence>
          {showTutorial && (
            <motion.div
              ...
              role="dialog"
              aria-modal="true"
              aria-label="Caribbean Stud Tutorial Modal"
            >
              <motion.div
              ...
              >
                ...
                <h3 className="text-xl font-bold text-cyan-400 mb-6 flex items-center gap-3 font-poppins">
                  <Users className="w-6 h-6 text-cyan-400 animate-pulse" /> Caribbean Stud Tutorial
                </h3>
