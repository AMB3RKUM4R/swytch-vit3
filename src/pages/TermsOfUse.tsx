...
// Placeholder TermsContent component
const TermsContent: FC = () => (
  <SwytchCard gradient="from-rose-500/20 to-cyan-500/20" className="max-w-4xl mx-auto p-8">
    <h2 className="text-2xl font-bold text-white font-poppins mb-4">Swytch PET Terms of Use</h2>
    <p className="mb-4 text-gray-300 font-inter">
      By using the Swytch Private Energy Trust (PET), you agree to these Terms of Use. The platform facilitates decentralized gaming and NFT trading, with no guarantees of financial returns.
    </p>
    <p className="mb-4 text-gray-300 font-inter">
      <strong>User Responsibilities:</strong> Users must comply with local laws regarding cryptocurrency and gaming. Swytch is not liable for any legal or financial consequences.
    </p>
    <p className="mb-4 text-gray-300 font-inter">
      <strong>Platform Usage:</strong> Access to games, NFTs, and membership features requires a connected wallet. All transactions are final once confirmed on the blockchain.
    </p>
    <p className="mb-4 text-gray-300 font-inter">
      <strong>Contact:</strong> For support, reach out to support@swytch.pet or join our community channels.
    </p>
  </SwytchCard>
);
...
const TermsOfUse: FC<ImportedPageProps> = ({
  userId,
  activeModal,
  setActiveModal,
  setShowMessage,
  ...
  const shareOnX = useCallback(async () => {
    ...
    try {
      const shareText = encodeURIComponent("Reviewed the Swytch PETverse Terms of Use! 📜 Join at swytch.io! #SwytchPETverse");
      ...
      setShowMessage('🎉 Shared Terms of Use on X! +5 JEWELS');
    } catch (err) {
  ...
  useEffect(() => {
    if (userId) {
      ...
      const unsubscribe = onSnapshot(userRef, (docSnap) => { // Renamed 'doc' to 'docSnap' for clarity
        ...
        setShowMessage('⚠️ Failed to load terms data.');
        setActiveModal('error');
      });
      return () => unsubscribe();
    } else {
      setShowMessage('⚠️ Please sign in to view terms!');
      setActiveModal('auth');
    }
        ...
        <motion.div className="relative z-10 max-w-6xl mx-auto py-16 px-6 sm:px-8 lg:px-16">
          <motion.div variants={sectionVariants}>
            <h1 className="text-4xl font-bold text-rose-400 flex items-center justify-center gap-3 font-poppins">
              <Sparkles className="w-8 h-8 text-cyan-400 animate-pulse" /> Terms of Use
            </h1>
            <p className="text-lg text-gray-300 max-w-2xl mx-auto mt-4 font-inter">
              Understand the rules governing your participation in the Swytch PETverse.
            </p>
          </motion.div>
          <motion.div variants={sectionVariants}>
            <TermsContent />
          ...
          <motion.div variants={sectionVariants} className="text-center py-8">
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              aria-label="Share Terms on X"
            >
              <MessageCircleHeart className="w-5 h-5 mr-2" /> Share Terms on X
            </motion.button>
            <Link
              to="/games"
